...
/***统一不同方式的回调函数***/
function tdxGetCallBack(formid, strFuncName, strErrNo, strResultCont) {
...
默认激活TQLEX调用方式***/
var activeServerType = function (type) {
...
function sendRequest(strCallWay, strFuncName, paramsFuncData, objOptParam, funcCallBack, addParObj) {
  ...
  if(addWay==='qs'){
    if(__tdxMobSystem === 'WEB-IOS'){
      activeServerType('ios');
    }else if(__tdxMobSystem === 'WEB-Android'){
      activeServerType('android');
    }
  }
  ...
  if (__tdxMobSystem == "IOS" || __tdxMobSystem == 'WEB-IOS') {
    ...
    if (strCallWay === "CallTQL") {
      ...
      _IOSBridgeConnect.call(strCallWay, cparams, strFuncName, formid, 'tdxGetCallBack');
    }
...
/***新增加密接口**/
//{EncryptType:0,EncryptedString:str}  0,des加密
function __tdxGetEncryptAcc(param,callback) {
  var webCallTql = new WEBCallTql();
  webCallTql.send('tdxGetEncryptAcc',param||{}, function (data) {
    callback(data);
  });
}
